...
function bpm(field, λ0, Lx, Ly, z, n=1; window=true, amplitude_array=ones(size(field)..., length(z)), paraxial=true)
	@assert ndims(field) == 2 "field needs to be 2 dimensional"
	...
	# this access the size of the first dimension
	Ny = size(field, 1)
	dy = Ly / Ny
...
# ╔═╡ 820b00bf-2856-4dda-9464-1c1699f9ed5f
width / Ly * N

# ╔═╡ ec7fefe8-b818-4205-940e-88b99bffe728
beam_rect = box((N, 1), (26, 1))

# ╔═╡ 34b64a24-ffcf-4467-8688-4667f9b3812e
size(zeros((10,)))

# ╔═╡ 361e4d1f-4263-41f1-a090-d6d8daa1860d
plot(y, beam_rect)

# ╔═╡ 00c44e0c-56be-462d-a525-eba14496e03f
phase_shift = cis.(2π ./ λ .* dz .* (fibre .- n_cladding));

# ╔═╡ 9fa39708-c158-4e58-816a-9036ad99bfd5
prop_rect = abs2.(bpm(beam_rect, λ, Lx, Ly, z));

# ╔═╡ b8c2b94c-1407-439b-9512-ae94162a5288
prop_rect_fibre = abs2.(bpm(beam_rect, λ, Lx, Ly, z, n_cladding, amplitude_array=phase_shift));

# ╔═╡ d07ac6d0-cd50-45ed-ae42-865d3ef2afe8
@bind iz Slider(axes(prop_rect,3 ), show_value=true, default=500)

# ╔═╡ 9b6eb2e3-261b-4479-ae58-08debc3de071
begin
	plot(y, prop_rect[:, 1, iz], label="rect free space", title="Distance in $(round(z[iz]*1000, digits=2)) mm")
...
beam_gauss = gaussian((N, 1), sigma=10)

# ╔═╡ 7cfe9115-3e5e-4d87-ab5e-058eff481441
prop_gauss = abs2.(bpm(beam_gauss, λ, Lx, Ly, z));

# ╔═╡ 454faa14-3f08-45b5-849d-2de0573840a6
prop_gauss_fibre = abs2.(bpm(beam_gauss, λ, Lx, Ly, z, n_cladding, amplitude_array=phase_shift));
...
# ╔═╡ 7fd5a666-46d2-4064-b097-bab6ee4dfa8c
size(prop_rect_fibre)

# ╔═╡ 3f7de961-6cd0-4dae-b0a5-0aa8e084a6fe
sum(prop_rect_fibre, dims=1)[1,1, :]

# ╔═╡ 4ca7ef14-f351-4663-819f-2c2d31fdbedb
begin
	plot(z, sum(prop_gauss_fibre, dims=(1,2))[:], xlabel="distance in m", ylabel="normalized intensity")
	plot!(z, sum(prop_gauss, dims=(1,2))[:])
end
...
#beam_multi_seed = gaussian((N,1), sigma=30) .* (1 .- box((N,1), (20,)))

# ╔═╡ 2f340646-9d8a-4b0c-b88f-61a76c48128d
beam_multi_seed = reshape((-width_multi / 2 .≤ y .≤ width_multi/2) .* sin.(y / width_multi * 2π), (N, 1));

# ╔═╡ 10207699-534c-4ef3-b832-1a45e70f153a
prop_multi_fibre = abs2.(bpm(beam_multi_seed, λ, Lx, Ly, z, n_core, amplitude_array=cis.(2π ./ λ .* dz .* (fibre_multi .- n_core)), paraxial=true));
...
prop_beam_3 = bpm(beam_3, λ, Lx, Ly, z2, n_core, amplitude_array=cis.(2π ./ λ .* dz .* (fibre_multi .- n_core)), paraxial=true);

# ╔═╡ dec1c5d6-de08-4601-965e-ccda9fb2ad5d
size(prop_beam_3)

# ╔═╡ bd8aec79-a435-4267-aa19-b79edce0c23a
prop_beam_3_back = conj.(bpm(conj.(prop_beam_3[:, :, end]), λ, Lx, Ly, z2, n_core, amplitude_array=reverse(cis.(2π ./ λ .* dz .* (fibre_multi .- n_core))), paraxial=true));
...
# ╔═╡ a6e9d81b-81bb-47eb-9aad-0c9c64cba492
md"## Answer
The NA of the fibre is calculated with

$$\text{NA} = \sqrt{n_\text{core}^2 - n_\text{cladding}^2}$$


From that we can calculate the maximum frequency the fibre transmit.
We can calculate a bandpass filter and convolved with it.
Then we see that the fibre acts as a lowpass filter.

"

# ╔═╡ 5421b519-6e2c-4e0b-982f-e578c1e08350
NA = sqrt(n_core^2 - n_cladding^2)

# ╔═╡ 51696c9f-e096-41bf-8609-f1e055a600c4
freqs = fftfreq(N, inv(Ly / N))

# ╔═╡ 82ddbdd3-e7d0-468e-871c-9fb8d88af4c7
freq_cut_off = abs.(freqs) .< NA / λ

# ╔═╡ d521af6c-11e0-4c3b-8df9-8b8770abae59
plot(fftshift(freqs), fftshift(freq_cut_off))

# ╔═╡ 7647e94e-e999-4248-9afe-8e056d90d52a
beam_3_filtered = ifft(fft(beam_3) .* freq_cut_off);

# ╔═╡ f9ac9bbe-5ce1-456f-a4b4-b3315fe79a16
@bind iz6 Slider(axes(prop_beam_3,3 ), show_value=true, default=size(prop_beam_3, 3))

# ╔═╡ aa651deb-11ff-4376-ab88-2929ede61761
begin
	plot(y, abs2.(prop_beam_3_back[:, 1, iz6]), label="Backpropagated", title="Distance in $(round(reverse(z2)[iz6]*1000, digits=2)) mm")
	plot!(y, abs2.(prop_beam_3[:, :,1]), label="initial beam")
	plot!(y, abs2.(beam_3_filtered[:,1]), label="filtered beam")
