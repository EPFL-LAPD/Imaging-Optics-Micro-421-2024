...
# ╔═╡ 54b8edb2-e933-4cb1-a4cd-a0a9871c0762
md"# 1. Off Axis Hologram

In this task we solve a problem around digital off-axis holography.

For that you are given an unknown hologram.

It was obtained by adding an olique illumination Gaussian beam to a sample beam. 
The sample beam propagated already a distance `z1` before it is added to the Gaussian beam.
This resulting signal was measured with image detector `abs2.`.

To reconstruct the signal, we analyze the Fourier spectrum of this `hologram` $|S+G|^2$. Use `ft` to Fourier transform the signal.
You will see there is a DC term and the 1st order and -1st order spots.

Take now an `extracted_region` of this Fourier spectrum centered around the 1st order.
Use `select_region` with the keywords `center` and `new_size`.
What is the theoretical center size? Note, it depends on the angle `α` and the wavelength `λ1`. How does the `new_size` affect the reconstruction? What size is good?

Inverse Fourier transform this subpatch back to real space.
Then use `free_space_propagation`to backproject this signal to the real sample plane.
What can you see?
"

# ╔═╡ 9502277f-6e64-422c-ac8c-2f424ad48456
urldownload("https://felix.sumpi.org/holography.png")

# ╔═╡ 77aed088-74c7-45fa-b6ca-574668b11fe3
illumination1 = gaussian((512, 512), sigma=200);

# ╔═╡ 77c09c80-0744-4bd9-8b66-b240bd81791a
# the hologram is produced with
#hologram2 = abs2.((sample .+ illumination1 .* exp.(1im .* k1 .* x .* sind(α))));

# ╔═╡ 64296df3-b864-47dc-8987-a02e07a8f987
hologram = Float32.(Gray.(urldownload("https://felix.sumpi.org/hologram2.png")));

# ╔═╡ 9bd699cd-87e7-49d6-9077-ee9483e4f8b3


# ╔═╡ 619c6ad2-e4a8-4382-abfd-ea6e3f5bf7f1
λ1 = 633e-9

# ╔═╡ 185020c9-dfff-4f8d-a947-08185a43d3b3
α = 10

# ╔═╡ ae999053-6e56-48a9-8b24-87bc7da18d30
k1 = 2π / λ1

# ╔═╡ ece00f0e-06bc-408b-b059-18422a3e541b
k1 .* sind(α) / (2π)

# ╔═╡ 83d4fad1-bf0a-45e8-a23a-6ee06b588c1a
z1 = 5e-3

# ╔═╡ ec93c3d6-af40-40cd-b11a-bc1797e6830b
L1 = 500e-6

# ╔═╡ ec947b37-30cf-49ab-8773-8a7ea0263a92
collect(fftshift(fftfreq(512, inv(L1 / 512))))

# ╔═╡ bc206b33-b7ba-4d70-9295-e5be55b7c92c
x = fftpos(L1, 512, CenterFT)

# ╔═╡ 4b1497cf-7de9-4993-b48a-ce1d81c5de2a
simshow(hologram)

# ╔═╡ 69fef15e-cb8c-4d41-b513-c11a0b747850
simshow(hologram)

# ╔═╡ 88595522-0701-4f47-a9ed-9ea678f9cab8


# ╔═╡ 67172e26-18d6-4664-aa74-74f2d34c8628


# ╔═╡ a8f96784-69b2-4000-9ff0-e1cd0f7272fa


# ╔═╡ f29dc027-98c0-429e-a10a-95bece1cb4af
simshow(ft(hologram), γ=0.1)

# ╔═╡ 94e29e9c-0a5c-4834-8f49-f530a4c4d43d
lol = randn((4,4))

# ╔═╡ 9a58658d-820d-4f51-8789-4eebb045f351
circshift(lol, (2,2))

# ╔═╡ b1da8358-4774-4c35-bdf4-85f54e8b0fee
size(hologram)

# ╔═╡ 477929cd-763a-4f48-be63-3e9aaddff183
extracted_part = select_region(ft(hologram), new_size=(300, 300), center=(120, 257));

# ╔═╡ e62694a6-f2a4-4cf9-b549-700916d5fa49
simshow(extracted_part)

# ╔═╡ 3c25de7d-e36d-4be3-9fd4-7065b9b8f838
reconstructed = free_space_propagation(ift(extracted_part), -z1, λ1, L1);

# ╔═╡ eb376044-da0e-4628-945c-6333fa814da5
simshow(abs.(reconstructed))

# ╔═╡ 4f34d1cc-efd0-4df7-b6ab-bf80cc4519e9


# ╔═╡ 96d3517c-7c7e-4379-bcfb-eb4ec720d391


# ╔═╡ ce94f523-ebde-4bb8-9f8f-07544553b10a


# ╔═╡ 0a016095-356a-46a2-96cf-db843864471c
md"# 2. Gerchberg-Saxton Algorithm

In this task you implement the [Gerchberg-Saxton algorithm](https://en.wikipedia.org/wiki/Gerchberg%E2%80%93Saxton_algorithm) for phase retrieval from an intensity measurement and a known illumination.

You are given the intensity `measurement`. The illumination is given by `illumination`.

Instead of using a Fourier transform (as in the image below), we use a free space propagation of `z=200e-3 m`. To forward propagate, use `free_space_propagation` with `z` and for back propagation use `-z`.
The functions `angle.` and `sqrt.` might be useful to replace the known amplitude at the image plane and diffraction pattern plane.
"

# ╔═╡ c5d92bfa-70f0-406a-942e-ac8fee0077f6
urldownload("https://upload.wikimedia.org/wikipedia/commons/thumb/f/f6/Gerchberg-Saxton_algorithm.jpg/1200px-Gerchberg-Saxton_algorithm.jpg")

# ╔═╡ 9526ac26-acee-4cf4-84cf-5d439c59f547
"""
	gerchberg_saxton(measurement, illumination, z, λ, L; N_iterations=50)

Use Gerchberg-Saxton to reconstruct the complex sample which is illuminated by `illumination`. The field size is `L` and the propagation distance `z`.
`measurement` is the intensity measurement of the final field.
This code does `N_iterations` iterations.
"""
function gerchberg_saxton(measurement, illumination, z, λ, L; N_iterations=50)
	field = illumination .* cispi.(1 .* rand(size(measurement)...))

	for i ∈ 1:N_iterations
		field_p = free_space_propagation(field, z, λ, L)
		field_p_new = sqrt.(measurement) .* cis.(angle.(field_p))

		field_back = free_space_propagation(field_p_new, -z, λ, L)
		field .= illumination .* cis.(angle.(field_back))
		#field .= illumination .* field_back
...
# ╔═╡ c28045cd-6968-4694-b517-552c81cc2cb6
measurement = Float64.(Gray.(urldownload("https://felix.sumpi.org/midterm_exam_img.png")));

# ╔═╡ dc2e2d54-90fe-4b7f-a18f-3a9b769df7aa
simshow(measurement)

# ╔═╡ f5205f51-0087-44c2-b04d-344ac6f5b187
illumination = box((128, 128), (80, 80));

# ╔═╡ fbf44fb3-9e80-43b9-9576-bf56f914dd06
λ = 633e-9

# ╔═╡ 6f16bec1-1608-44a7-b429-304c220aa99e
L = 4e-3

# ╔═╡ 8d51a07c-15cb-4127-8665-4296fd371c63
z = 200e-3

# ╔═╡ 86a31bf0-9120-495c-bff0-f14b4f6cfc66
@time field_initial = gerchberg_saxton(measurement, illumination, z, λ, L, N_iterations=100);

# ╔═╡ b8daa1bb-d97a-4895-b0a5-734c22fe2e4a
simshow((field_initial) .* cis(5))

# ╔═╡ 137a6b7f-36ef-4153-9c1f-2d05294ecce8
md"# Extra task"

# ╔═╡ df2960c9-5c02-4cbc-a87d-e37325ed9ea1
gabor = select_region(select_region(Float64.(urldownload("https://optica-org-web-afd-f9abf4byhbacgfgk.z02.azurefd.net/optica/media/osa.media/headshots/dgabor_1972.jpg?t=637811404694082402")[200:500, 81:480])), new_size=(256,256));

# ╔═╡ e0ab6ab7-a2f7-4a36-b978-0a7a83526de2
psaltis = select_region(Float64.(Gray.(urldownload("https://people.epfl.ch/private/common/photos/links/174684.jpg?ts=1715168229"))), new_size=size(gabor));

# ╔═╡ 01509cd0-25d0-4b34-a350-f16e45534f94
simshow(gabor)

# ╔═╡ 880040c7-3d9a-4b8e-8647-68af1ee34ed1
simshow(psaltis)

# ╔═╡ ffc5f3c2-c0ad-4f58-8c00-8450845dc1fc
md"
Find a complex field where the abs of the field at on is gabor
and at the other is Psaltis.

"

# ╔═╡ c3c63fff-52af-4719-95cc-a0687c4f0001
function match_psaltis_gabor(psaltis, gabor, N_iterations=50)
	guess = ones(ComplexF64, size(psaltis))

	prop(x) = free_space_propagation(x, 0.7e-3, 633e-9, 1e-3)
	prop_back(x) = free_space_propagation(x, -0.7e-3, 633e-9, 1e-3)

	for i ∈ 1:N_iterations
		guess = abs.(gabor) .* cis.(angle.(guess))
		guess_ft = prop(guess)
		guess_ft = abs.(psaltis) .* cis.(angle.(guess_ft))
		guess = prop_back(guess_ft)
	end
	return guess
end

# ╔═╡ 633e1328-ae66-4e22-81e2-2813daec0b98
hologram_psaltis_gabor = match_psaltis_gabor(psaltis, gabor, 100);

# ╔═╡ 9f8abc20-8c4b-471e-a826-96935ba9664e
simshow(abs.(hologram_psaltis_gabor))

# ╔═╡ 02ce0318-77cc-4622-bf7b-418cebf1ddae
simshow(abs.(free_space_propagation(hologram_psaltis_gabor, 0.7e-3, 633e-9, 1e-3)))

# ╔═╡ f735ea90-51c1-4959-8858-db2d7c20835f
simshow(abs.(ft(hologram_psaltis_gabor)))

# ╔═╡ edd58750-fd85-441e-bd39-eaaecae2f30b
simshow(hologram_psaltis_gabor)

# ╔═╡ 7e350674-90e2-45fc-9e87-737e450fcf2d
simshow(ft(hologram_psaltis_gabor))
