...
# ╔═╡ f1abb608-c56e-4a88-84ef-0d1cf3d97050
using LinearAlgebra
...
# ╔═╡ 50cf4730-3a0e-44a6-b3c0-b85281bac209
...
# ╔═╡ 3051cffc-ff2b-4250-b930-15c5b9c0921f
img = Float32.(select_region(testimage("resolution_test_512"), center=(250, 320), new_size=(100, 100))[begin:2:end, begin:2:end]);

# ╔═╡ fba88879-6300-462f-bd64-e2fa844d467c
simshow(img)

# ╔═╡ f1c78448-5c13-490a-8dc2-945ab1ab6759
N = size(img, 1)

# ╔═╡ 9bf4273e-7b55-4cce-8ead-243f0e3e35c2
z = 200e-6

# ╔═╡ 0976061a-1720-4b9e-97da-5dae3b1e441c
λ = 633e-9

# ╔═╡ 13b20910-1952-4a46-8b48-3bc1d42185ff
L = 50e-6

# ╔═╡ b973b266-8993-4db0-9074-bf23a00e6cee
diffuser = conv_psf(cis.(2 .* randn(size(img)...)), gaussian(size(img), sigma=1.5));

# ╔═╡ 631cd34f-c3f6-42ee-8a6d-a922f3002948
simshow(diffuser);

# ╔═╡ 550365b8-75c9-46ff-bfd1-7aac851bbaef
fwd(x) = free_space_propagation(x, z, λ, L)

# ╔═╡ c7f55837-40bb-44c2-90f7-5cbffbad689b
md"# 2. Measure Linear Matrix


Usually optical systems are linear systems hence they can be described with matrices.
In practice this is used for complex systems such as fibres. Imaging systems are better described with more efficient approaches such as Fourier transforms.

To measure the system matrix $M$ experimentally, we probe the system with $K$ different inputs $\vec x$. Each $\vec x$ is one projected image, we just reshape as a vector.
Then, we assemble our input matrix $X$ where each column corresponds to one input vector.





So $X \in \mathbb{C}^{N^2 \times K}$. 
The system matrix is $M \in \mathbb{C}^{N^2 \times N^2}$.
The measurements are $Y \in \mathbb{C}^{N^2 \times K}$.  
$N^2$ because we reshape a image with size $N \times N$ into a vector of length $N^2$.


The connection between input $X$ and output matrix $Y$ is:

$$Y = M \cdot  X$$.


To obtain $M$ we invert $X$.
The inverse of $X$ is not possible since $X$ is not quadratic and also not of full rank.
We have to use the pseudoinverse for it.

$$M = Y \cdot X^{-1}$$.

$$M = Y \cdot \text{pinv}(X)$$

By choosing suitable inputs $X$ we hope to find a matrix $M$ which is able to represent the optical system.
"

# ╔═╡ b5cc0ed6-eb23-4908-bb50-492e67b74348
function make_measurements(fwd, N, K)
	
	Y = zeros(ComplexF64, N^2, K)
	X = zeros(ComplexF64, N^2, K)
	
	for c in 1:K
		# create a random direction Gaussian Beam
		dir1 = randn() / 10
		dir2 = randn() / 10
		xpos = fftpos(L, N)
		#diffuser2 = conv_psf(cis.(3 .* randn(size(img)...)), gaussian(size(img), sigma=1.5));
		# a single vector which we use to probe the matrix
		x = gaussian((N, N), sigma=15) .* exp.(2π .* 1im ./ λ .* (xpos .* sin(dir1) .+ xpos' * sin(dir2)))

		# save input x to X
		X[:, c] .= reshape(x, :)
		# use optical system to propgate x and save it to Y
		Y[:, c] .= reshape(fwd(x), :)
	end
...
# ╔═╡ 60eb88aa-608d-4a0d-b862-6a7761f7ab1b
N

# ╔═╡ 6e5e42b6-7a81-400a-b325-fcc15c6051f8
N^2

# ╔═╡ f903a341-5d1f-4aa2-a3f8-dbbf1076d29e
K = 1000

# ╔═╡ d69fc6db-9747-4d4b-be12-2f162e350a49
X, Y = make_measurements(fwd, 50, K)

# ╔═╡ 20fc69f1-daaa-4074-8e0b-636f293231e1
size(X)

# ╔═╡ 5e8702b6-f4bb-43e9-ad57-6df9a09599bf
M = Y * pinv(X);

# ╔═╡ f7ce5e1d-fcae-4375-b6a7-8196cdda2900
@bind k2 Slider(1:K)

# ╔═╡ 9da248b2-cfcb-4cf4-ab3c-51c49dd46e98
simshow(reshape(X[:, k2], N, N))

# ╔═╡ 28673109-5cdd-4a6c-959d-bbeafe2dd8be
simshow(reshape(M * X[:,k2], N, N))

# ╔═╡ d19cc836-1b70-4ca9-9463-f395caccc42d
simshow(fwd(reshape(X[:, k2], N, N)))

# ╔═╡ 4d0e7787-be34-4fcb-a55c-3d83678a59e1
md"# 3. Propagate object with matrix
Output seems to match well.
"

# ╔═╡ 5bc0ade2-7e89-41e4-9daa-926fb2975de2
simshow(img)

# ╔═╡ d65764cb-fdf4-4e98-8daa-b617d9f0e86f
simshow(reshape(M * reshape(img, :), N, N))

# ╔═╡ ff517230-dccd-4ece-bc67-3370a36eb068
simshow(fwd(img))

# ╔═╡ 81f7c46f-773f-4c0c-ac75-df02fdd2ecc7
# inverting M does not work because it's not a full rank matrix, hence we use a pseudoinverse
inv2M = inv(M' * M + 0.1 * I) * M';

# ╔═╡ 1d1c09e2-0a3b-4ebf-94db-227d4a89ae63
simshow(reshape(inv2M * reshape(fwd(img), :), N, N))

# ╔═╡ f1d204d7-c4be-4365-9013-afce2137c8d9
simshow(img)
...
manifest_format = "2.0"
project_hash = "667e6efebef622065884811bd688e571a01ffe90"
...
[[deps.DiffImageRotation]]
deps = ["Atomix", "ChainRulesCore", "KernelAbstractions"]
git-tree-sha1 = "bb3efab67a0e5fee71d06d94290deabf806cd93e"
uuid = "cb1f95eb-7c3a-45bc-9e18-5c07f1beaacd"
version = "0.3.0"
...
git-tree-sha1 = "28c1558936dd80dd207530921a960873e6056b45"
