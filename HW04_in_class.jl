...
# ╔═╡ 91b12ebc-cec0-4afa-a4cc-57a91e136540
...
z1 = range(0, focal_length, 150)

# ╔═╡ 8585dc8b-a7e7-42ed-b755-65b8e6a410b8
prop1 = bpm(grating, λ, L, L, z1);

# ╔═╡ 8964b28b-6add-4324-96e0-3242a5caf76c
z1[end]

# ╔═╡ 733dca27-a8dd-45fa-9318-970b3a76516d
z2 = range(0, focal_length + 50f-3, 200)

# ╔═╡ 98b619f2-784a-4fab-bfc6-b5a5d3c2f9cc
simshow((exp.(-1im .* π ./ λ ./ (focal_length) .* (x.^2 .+ y.^2))))
...
prop2 =  bpm(field_after_lens, λ, L, L, z2);

# ╔═╡ cf0b7836-93ca-402c-a078-3b1235e536b8
total_field = abs2.(cat(prop1, prop2, dims=3));

# ╔═╡ c1e99bfd-efe5-46d0-8f88-095016984b90
heatmap(vcat(z1, z1[end] .+ z2), y, abs.(total_field[129, :, :]).^0.2, cmap=:turbo, xlabel="propagation distance in m", ylabel="size in m",)
...
begin
	plot(x[:], abs.(total_field[129, :, 300] * 200), xlabel="distance in mm")
	plot!(x[:], abs.(ft(grating)[129, :]))
end

# ╔═╡ 96e5321c-b23a-4387-a34f-fda20bf6b3f6


# ╔═╡ d901e87a-f67f-49f8-96bf-7c7f84e74c98
md"""
## Answer
Why is the scaling of `ft` correct?

Because we have choosen parameter that the scaling is exactly 1!

See `L` and `L_after_ft` have the same value!
"""

# ╔═╡ 7c5e7508-0f45-4668-938f-9f6a8c52e776
L_after_ft = λ * focal_length * N / L

# ╔═╡ ad1db870-ce85-43e8-ad0d-1087d2660eb0
L
...
img_aperture = abs2.(ft(ft(img) .* circ_aperture));
...
md"
### Solution
The highest frequency given by Nyquist corresponds to 
$N / L / 2 = 256000 m^{-1}$.

The frequency of the sinusoidal grating is $\frac{1}{8 \cdot L_2 / N_2}= 64 000 m^{-1}$

The full aperture radius corresponds to $256000 m^{-1}$ and 256 pixels, since the Fourier space is symmetric.
So the radius of the aperture needs to be 

$$\frac{256000m^{-1}}{(64000m^{-1})} \cdot 256\text{pixels} ≈ 64\text{pixels}$$

In physical sizes, this means that the radius covers 64/512 pixels.
And the size in Fourier space is 

$$\frac{\lambda_2 \cdot f_2 \cdot N_2}{ L_2} ≈ 16.2mm$$

So the aperture radius is $4.05\mathrm{mm}$

"

# ╔═╡ e3fd37e0-92c7-47d0-b3be-cbc4388bdba8
L2

# ╔═╡ 9761bedf-7acb-47ee-9da6-eabf4cc31b34
inv(64000)

# ╔═╡ 4e05696f-f6cd-4e33-b9f3-48b1f14ca41e
λ2 * f2 * N2 / L2

# ╔═╡ 27c52e92-4240-4182-a400-7d523882ab99
λ2 * f2 * N2 / L2 / 4 / 2
...
@bind radius_inner Slider(0:radius_outer, show_value=true)

# ╔═╡ 5eb45850-fa25-49f9-b6c6-8ad8337ee714
annulus = 1.0 .* (rr(size(img)) .<= radius_outer) .- 1 .* (rr(size(img)) .<= radius_inner);

# ╔═╡ 64a2924f-011d-477c-8931-fa7cee6c899b
simshow(annulus)

# ╔═╡ c3893fd5-9ae3-49f5-bf2e-8ce4119703cd
img_annulus = abs2.(ft(ft(img) .* annulus)); 

# ╔═╡ 48075c22-0a33-4dbd-b200-f6df9fcafc7c
simshow(img_annulus, cmap=:turbo, γ=0.9)
...
### Answer
The donut blocks high and low frequencies. Since the Siemens star has high frequencies in the center and low frequencies on the outside, the Siemens star also looks like a donut!
...
Your pupil has a diameter of 5mm and the focal length is 23mm.

The numerical aperture of the eye is: $\text{NA} \approx \frac{5\textrm{mm}}{23\textrm{mm} \cdot 2} = 0.11$

The minimal resolution at the retina is given by: $\Delta x = \frac{\lambda}{2\text{NA}} \approx 1.8\mathrm{\mu m}$.


The magnification is given by $\frac{z_2}{z_1}$ where $z_2 = 380.000\mathrm{km}$ is the distance from Moon to the earth. and $z_1$ is the focal length of the eye.
$M \approx 1.6 \cdot 10^{10}$


Hence, the minimal feature at the earth is: $M \cdot \Delta x \approx 30 \textrm{km}$

Since the lake is around $50\text{km}$ long, we should be able to see it.
